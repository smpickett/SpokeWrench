<?php
//=========================================================================
// FILE: WeatherFunctions.php
// Copyright (c) 2011 Stephen Pickett
//=========================================================================

//====== Constants ========================================================
$WU_CurrentPwsData = 'http://api.wunderground.com/weatherstation/WXCurrentObXML.asp?ID=';
//====== ENF OF Constants =================================================

if(!isset($_POST['func']))
{
  $return['error'] = true;
  $return['errormsg'] = "[PHP ERROR] Missing function specification in POST [".__FILE__."][".__FUNCTION__."]";
...
switch($_POST['func'])
{
  case 'GetWeatherStations':
    GetWeatherStations();
    break;
  case 'GetWeatherData':
    GetWeatherData();
    break;
  case 'GetWeatherStationLatLon':
    GetWeatherStationLatLon();
    break;
  default:
    $return['error'] = true;
    $return['errormsg'] = "[PHP ERROR] Invalid function specification in POST [".__FILE__."][".__FUNCTION__."]";
    echo json_encode($return);
    return;
}
...
// Arguments:   
//----------------------------------------------------------------------
function GetWeatherStationLatLon()
{
  global $WU_CurrentPwsData;

  // Initialize the basic return array
  $return['error'] = false;
  $return['errormsg'] = "";
  ...
  {
    $return['error'] = true;
    $return['errormsg'] = "[PHP ERROR] POST missing 'id' variable [".__FILE__."][".__FUNCTION__."]";
    echo json_encode($return);
    return;
  }

  // Find the station data
  $xmlstring = $WU_CurrentPwsData.$_POST['id'];
  $xml = simplexml_load_file($xmlstring);
  if(is_null($xml))
  {
    $return['error'] = true;
    $return['errormsg'] = "[PHP ERROR] xml string (".$xmlstring.") was not responsive [".__FILE__."][".__FUNCTION__."]";
    echo json_encode($return);
    return;
  }
  ...
  foreach($xml->children() as $child)
  {
    ...
    {
      foreach($child->children() as $stationElement)
      {
        switch($stationElement->getName())
        {
          case 'latitude':
            $return['lat'] = $stationElement."";
            break;
          case 'longitude':
            $return['lon'] = $stationElement."";
            break;
        }
      }
    }
...
function GetWeatherStations()
{
  ...
  {
    $return['error'] = true;
    $return['errormsg'] = "[PHP ERROR] POST missing 'xml' variable [".__FILE__."][".__FUNCTION__."]";
    echo json_encode($return);
    return;
  }
  ...
  {
    $return['error'] = true;
    $return['errormsg'] = "[PHP ERROR] xml string (".$_POST['xml'].") was not responsive [".__FILE__."][".__FUNCTION__."]";
    echo json_encode($return);
    return;
  }
            ...
            foreach($station->children() as $stationElement)
            {
              switch($stationElement->getName())
              {
                case 'id':
                  ...
                  break;
                case 'distance_km':
                  array_push($station_dist, $stationElement."");
                  break;
              }
            }
          }
  ...
  $return['idarray'] = $station_ids;
  $return['distarray'] = $station_dist;
  echo json_encode($return);
  return;
}
